...
-- This migration is idempotent enough for a single production application.
...
begin
  ...
  update public.debts
  set status = case when v_paid >= v_original then 'paid' else 'active' end
  where id = p_debt_id;
end;
$$;
...
begin
  ...
  select
    ...
    t.transaction_date,
    'transaction',
    'Automatisk matchet via ' || v_debt.match_text
...
begin
  if tg_op = 'UPDATE' then
    ...
    where transaction_id = new.id and source = 'transaction';
  end if;

  if new.amount >= 0 then return new; end if;

  select count(*), min(d.id::text)::uuid
  into v_matches, v_debt_id
  from public.debts d
