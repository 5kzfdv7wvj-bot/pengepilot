// PengePilot web boot v15: four main areas, savings + debt tracking, no extra AI page.
(() => {
  const P = window.pp13;
  if (!P || window.__PP15_BOOT__) return;
  window.__PP15_BOOT__ = true;
  ...
  const defs = {
    ...
    save: { tabs: [['suggestions', 'Forslag'], ['debts', 'Gæld'], ['fixed', 'Faste'], ['plan', 'Budget & mål']], def: 'suggestions' }
  };
  const original = {};
  let installed = false;
  ...
  const tabs = group => `<div class="pp13tabs pp14tabs" role="tablist">${defs[group].tabs.map(([id, label]) => `<button type="button" class="pp13tab ${activeTab(group) === id ? 'on' : ''}" onclick="pp15Tab('${group}','${id}')">${esc(label)}</button>`).join('')}</div>`;
  const clean = html => String(html || '').replace(/<div id="modal"><\/div>/g, '');

  window.pp15Tab = (group, tab) => {
  ...
  async function saveHub() {
    ...
    if (tab === 'debts') return `${tabs('save')}<div class="pp14hub">${await original.debts()}</div>`;
    if (tab === 'fixed') {
  ...
  function needed() {
    ...
    const req = {
      ...
      savings: ['savings', 'debts', 'subscriptions', 'bills', 'budget'],
      settings: ['settings'],
      import: []
    }[page] || [];
  ...
  async function install() {
    ...
    titles.savings = ['Spar penge', 'Forslag, gæld, faste udgifter og mål'];
  ...
  const timer = setInterval(async () => {
    tries++;
    try {
      if (await install() || tries > 180) clearInterval(timer);
    } catch (error) {
      console.error('PengePilot v15 boot', error);
      clearInterval(timer);
    }
  }, 30);
})();
